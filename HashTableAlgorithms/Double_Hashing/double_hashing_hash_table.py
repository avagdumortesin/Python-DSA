"""Implement a hash table using double hashing."""

from __future__ import annotations

from HashTableAlgorithms.shared.hash_table import HashTable
from HashTableAlgorithms.shared.map_adt import K, V
from HashTableAlgorithms.shared.open_addressing_bucket import OpenAddressingBucket
...
class DoubleHashingHashTable(HashTable[K, V]):
    """Implement an open-addressing hash table with double hashing."""
    ...
    def bucket_index(self, key: K, offset: int = 0) -> int:
        """Return the bucket index for a double-hashing offset."""
        return (self.compute_hash(key) + offset * self._secondary_hash(key)) % len(
            self.table
        )

    def _secondary_hash(self, key: K) -> int:
        """Return the step size produced by the second hash function."""
        return 7 - self.compute_hash(key) % 7
