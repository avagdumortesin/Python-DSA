...
The implementation distinguishes between buckets that have never contained an item and buckets that became empty after removal. This allows searches to continue through deleted positions without incorrectly terminating a probe sequence. It also shares common map and hash table abstractions with the other collision-resolution strategies in this repository, allowing each implementation to focus on its collision-resolution strategy.
