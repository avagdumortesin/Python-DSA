...
class LinearProbingHashTable(HashTable[K, V]):
    """Implement an open-addressing hash table with linear probing."""
    ...
    def bucket_index(self, key: K, offset: int = 0) -> int:
        """Return the bucket index for a linear-probing offset."""
        return (self.compute_hash(key) + offset) % len(self.table)
