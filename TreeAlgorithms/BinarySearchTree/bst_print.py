from TreeAlgorithms.BinarySearchTree.bst_node import BSTNode
...
class BSTPrint:
    ...
    def _indent_lines(lines: list[str], number_of_spaces: int) -> None:
        ...
        if number_of_spaces > 0:
            indent = BSTPrint._get_spaces(number_of_spaces)
    ...
    def tree_to_lines(subtree_root: BSTNode | None) -> list[str]:
        ...
        if subtree_root.left is None:
            ...
            all_lines[1] = BSTPrint._get_spaces(node_text_len) + "\\"
            ...
            if right_child_indent <= node_text_len:
                ...
                BSTPrint._indent_lines(right_lines, node_text_len - right_child_indent)
            else:
                # Shift the current node instead.
                indent = BSTPrint._get_spaces(right_child_indent - node_text_len)
        ...
        if subtree_root.right is None:
            ...
            indent = BSTPrint._get_spaces(left_lines[0].find("]"))
        ...
        absolute_indent = BSTPrint._get_spaces(indent_needed)
        ...
        all_lines[1] = (
            BSTPrint._get_spaces(left_index)
            + "/"
            + BSTPrint._get_spaces(right_index - left_index - 1)
            + "\\"
        )
        ...
        if len(node_text) < available_width:
            ...
            if difference % 2 != 0:
                node_text += "_"
        all_lines[0] = BSTPrint._get_spaces(left_index + 1) + node_text
        return all_lines
