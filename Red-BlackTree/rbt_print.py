...
class RBTPrint:
    """Utility methods for creating a text representation of a red-black tree.

    This class converts a red-black tree into an ASCII-art representation
    ...
    def _indent_lines(lines: list[str], number_of_spaces: int) -> None:
        ...
        if number_of_spaces > 0:
            indent = RBTPrint._get_spaces(number_of_spaces)
            for i, line in enumerate(lines):
                lines[i] = indent + line

    @staticmethod
    def _node_to_string(node: RBTNode) -> str:
        """Return a printable representation of a red-black tree node.

        Args:
            node: The node to represent.

        Returns:
            The node's key and color enclosed in brackets.
        """
        color = "R" if node.color is NodeColor.RED else "B"
        return f"[{node.key} {color}]"

    @staticmethod
    def tree_to_lines(subtree_root: RBTNode | None) -> list[str]:
        """Convert a red-black tree into a list of printable text lines.
        ...
        node_text = RBTPrint._node_to_string(subtree_root)
        ...
        left_lines = RBTPrint.tree_to_lines(subtree_root.left)
        right_lines = RBTPrint.tree_to_lines(subtree_root.right)
        ...
        if subtree_root.left is None:
            ...
            all_lines[1] = RBTPrint._get_spaces(node_text_len) + "\\"
            ...
            if right_child_indent <= node_text_len:
                ...
                RBTPrint._indent_lines(right_lines, node_text_len - right_child_indent)
            else:
                # Shift the current node instead.
                indent = RBTPrint._get_spaces(right_child_indent - node_text_len)
        ...
        if subtree_root.right is None:
            ...
            indent = RBTPrint._get_spaces(left_lines[0].find("]"))
        ...
        absolute_indent = RBTPrint._get_spaces(indent_needed)
        ...
        all_lines[1] = (
            RBTPrint._get_spaces(left_index)
            + "/"
            + RBTPrint._get_spaces(right_index - left_index - 1)
            + "\\"
        )
        ...
        if len(node_text) < available_width:
            ...
            if difference % 2 != 0:
                node_text += "_"
        all_lines[0] = RBTPrint._get_spaces(left_index + 1) + node_text
        return all_lines

    @staticmethod
    def tree_to_string(subtree_root: RBTNode | None) -> str:
        """Convert a red-black tree into a printable string.
        ...
        lines = RBTPrint.tree_to_lines(subtree_root)
        return "\n".join(lines)
