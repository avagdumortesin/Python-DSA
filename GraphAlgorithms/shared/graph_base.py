"""Provide the shared foundation for graph implementations."""
...
class GraphBase:
    ...
    def get_edges(self) -> set[Edge]:
        """Return all distinct directed edges in the graph."""
        return {
            current_edge for edges in self.from_edges.values() for current_edge in edges
        }
    ...
    def get_edges_list(self) -> list[Edge]:
        """Return all distinct directed edges as a list."""
    ...
    def has_edge(
        ...
        return any(
            current_edge.to_vertex is to_vertex
            for current_edge in self.from_edges[from_vertex]
        )
